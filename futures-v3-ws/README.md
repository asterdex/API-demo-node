# Futures V3 WebSocket Demo / 期货V3 WebSocket示例

Complete examples for Futures V3 WebSocket streams.  
完整的期货V3 WebSocket流示例。
...
`15_userData.js` 会自动：
- 从 `futures-v3-demo/config.js` 读取配置
- 调用 REST API 创建 listenKey
- 连接到用户数据流
- 每30分钟自动延长 listenKey 有效期

The script automatically:
- Loads config from `futures-v3-demo/config.js`
...
⚠️ **Note**: User data stream requires valid API configuration in `futures-v3-demo/config.js`  
用户数据流需要在 `futures-v3-demo/config.js` 中配置有效的API信息
